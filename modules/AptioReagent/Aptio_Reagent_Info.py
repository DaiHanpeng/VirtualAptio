...
class InstrumentReagentInfo(object):
    ...
    '''
    def __init__(self, instr_id, instr_type, time_stamp=None, reagent_info_list=[], instr_name = 'Undefined_Name'):
        '''
        Instrument_Id: str
        Instrument_Type: str
    ...
    def update_reagent_info_item(self, reagent_name, reagent_count):
        ...
        if reagent_info:
            ...
            reagent_info = ReagentInfoItem(name=reagent_name,count=reagent_count)
            self.add_reagent_info_item(reagent_info)

    def update_reagent_status(self):
        self.reagent_status = GREEN
        for item in self.reagent_info_list:
            if isinstance(item, ReagentInfoItem):
                if self.reagent_status < item.reagent_status:
                    self.reagent_status = item.reagent_status
...
class AptioReagentInfo(object):
    ...
    def update_reagent_status(self):
        ...
        for instrument in self.reagent_info_table:
            if isinstance(instrument, InstrumentReagentInfo):
                if self.reagent_status < instrument.reagent_status:
                    self.reagent_status = instrument.reagent_status
